...
func TestUpdateUser(t *testing.T) {
	...
	router.HandleFunc("http://localhost:8080/api/user/%s", UpdateUser).Methods(http.MethodPut)

	for _, tt := range tests {
		byteData , _:= json.Marshal(tt.Person)
		//url := fmt.Sprint(tt.Url, tt.Person.Id)
		req, err := http.NewRequest(tt.Method, "http://localhost:8080/api/user/%s", bytes.NewReader(byteData))
		if err != nil{
			t.Fatalf("unable to create any request  : %v", err)
		}
		res := httptest.NewRecorder()
